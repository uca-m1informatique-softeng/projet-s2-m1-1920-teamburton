...
public class StrategieBatiment implements IntelligenceArtificielle {
    ...
    public Personnage choixRole(Partie partie, Plateau plateau, int nbDoublon, int nbTonneauxTotal,
        ...
        // S'il y a des carrières disponibles et que le joueur en possède moins de 4
        if (plateau.placeDispoIle() && plateau.getNbCarriere() < 4 && !partie.getCarrieres().isEmpty()){
    ...
    public Exploitation choixExploitation(Partie partie, Plateau plateau, boolean joueursActif,
        ...
        // Choix carrieres prioritaires
        if (plateau.getNbCarriere() < 4 && !carrieres.isEmpty()){
            return carrieres.remove(0);
        }

        // Choix plantations selon le bâtiments de productions possédé
    ...
    public Batiment choixBatiment(Partie partie, int nbDoublon, Plateau plateau) {
        ...
        for (int b = 0; b < batiments.size(); b++) {
            ...
            if (!plateau.possedeBatiment(Constantes.BARRAQUE) && batiment.getNom().equals(Constantes.BARRAQUE)
        ...
        for (int b = 0; b < batiments.size(); b++){
            ...
            if (plateau.possedeExploitationOccupee(batiment.getNomPlantation()) &&
                    !plateau.possedeBatiment(batiment.getNom()) && batiment.getPrix() <= nbDoublon) {
